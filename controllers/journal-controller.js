...
module.exports = {
  ...
  getOneEntry: function (req, res) {
    db.Journal.findOne({ _id: req.params.noteId, user: req.id })
      .then((dbData) => res.json(dbData))
      .catch((err) => res.status(422).json(err));
  },

  updateOneEntry: function (req, res) {
    db.Journal.findOneAndUpdate({ _id: req.params.noteId, user: req.id })
      .then((dbData) => res.json(dbData))
      .catch((err) => res.status(422).json(err));
  },

  removeOneEntry: function (req, res) {
    db.Journal.findOneAndDelete({ _id: req.params.noteId, user: req.id })
